...
import type { Scheme } from "./comms";

export type CommsConfig = {
  tab: "Analog" | "Constellation" | "BER Curve";
  analog: { type: "AM" | "FM"; fm: number; fc: number; idx: number };
  constellation: { scheme: Scheme; snr: number; n: number };
  ber: { scheme: Scheme };
};
...
export default function SavePreset({
  ...
  config: CommsConfig;
  onLoad: (config: CommsConfig) => void;
}) {
  const { user } = useAuth();
  ...
  if (!user) {
    return <span className="save-hint">Sign in to save setups (Pro feature)</span>;
  }
  ...
  async function save(e?: React.FormEvent) {
    ...
    setSaving(true);
    setMsg(null);
    const { error } = await supabase.from("comms_presets").insert({
      user_id: user!.id,
      name: name.trim(),
      config,
    });
  ...
  async function load(id: string) {
    setBusy(true);
    const { data, error } = await supabase.from("comms_presets").select("*").eq("id", id).single();
    setBusy(false);
    if (!error && data) {
      onLoad(data.config);
      setListOpen(false);
  ...
  async function remove(id: string, e: React.MouseEvent) {
    ...
    setConfirmId(null);
    await supabase.from("comms_presets").delete().eq("id", id);
  ...
  return (
    <div className="saveload">
      {naming ? (
        ...
        <button className="save-btn" onClick={() => setNaming(true)}>💾 Save Setup</button>
      )}
      <div className="load-wrap">
        <button className="save-btn" onClick={openList}>📂 My Setups {listOpen ? "▲" : "▼"}</button>
        {listOpen && (
          <div className="load-list">
            {busy && <p className="load-empty">Loading…</p>}
            {!busy && rows && rows.length === 0 && <p className="load-empty">No saved setups yet.</p>}
            {!busy && rows?.map((r) => (
